...
async fn get_journal_entries() -> Result<Vec<JournalEntryMetadata>, String> {
    ...
    if !entries_dir.exists() {
        fs::create_dir_all(&entries_dir).map_err(|e| format!("Failed to create directory: {}", e))?;
    }
            ...
            if let Ok(entry) = entry {
                ...
                if path.extension().and_then(|s| s.to_str()) == Some("md") {
                    if let Ok(content) = fs::read_to_string(&path) {
                        ...
                        let content_without_title = content.lines().skip(1).collect::<Vec<_>>().join("\n");
                        let preview = content_without_title.chars().take(150).collect::<String>() + 
                            if content_without_title.len() > 150 { "..." } else { "" };
...
async fn get_journal_entry(id: String) -> Result<JournalEntry, String> {
    let home_dir = dirs::home_dir().ok_or("Failed to get home directory")?;
    let entry_path = home_dir.join("Documents").join("Diaryx").join("entries").join(format!("{}.md", id));
    ...
    let metadata = fs::metadata(&entry_path).map_err(|e| format!("Failed to read metadata: {}", e))?;
    let modified_at = metadata.modified()
        .map_err(|e| format!("Failed to get modified time: {}", e))?
        .into();
